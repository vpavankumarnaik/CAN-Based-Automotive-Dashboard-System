...
void main(void)
{
    ...
    while (1)
    {
        ...
        can_transmit( 0x30 , (uint8_t *)&rpm, 2);
        delay(300);

        uint8_t cur_indicator = process_indicator();
        can_transmit( 0x40 , &cur_indicator, 1 );
...
{
    uint16_t adc_val = read_adc(0x04);
    uint16_t rpm = (adc_val / 10.23) * 60;
...
{
    static IndicatorStatus indicator_mode = e_ind_off;
    unsigned char key;
    ...
    if (key == 0x0E)
    {
        indicator_mode = e_ind_left;
    }
    else if (key == 0x0D)
    {
        indicator_mode = e_ind_hazard;
    }
    else if (key == 0x0B)
    {
        indicator_mode = e_ind_right;
    }
    else if(key == 0x07)
    {
        indicator_mode = e_ind_off;
    }
