...
void can_display();
void display_speed(uint8_t *data, uint8_t len);
void display_gear(uint8_t *data, uint8_t len);
void display_rpm(uint8_t *data, uint8_t len);
void display_indicator(uint8_t *data, uint8_t len);
# 8 "source.c" 2

const char *gear_msg[] = {"N ", "1 ", "2 ", "3 ", "4 ", "5 ", "R ", "_C"};
...
void can_display()
{
    ...
    if(can_receive(&msg_id, data, &len))
    {
        if(msg_id == 0x10)
        {
            display_speed(data, len);
        }
        else if(msg_id == 0x20)
        {
            display_gear(data, len);
        }
        else if(msg_id == 0x30)
        {
            display_rpm(data, len);
        }
        else if(msg_id == 0x40)
        {
            display_indicator(data, len);
        }
    }
}
...
void display_speed(uint8_t *data, uint8_t len)
{
    unsigned char rx_speed = data[0];

    clcd_putch((rx_speed / 100) + '0', (0xC0 + (0)));
    clcd_putch(((rx_speed / 10)%10) + '0', (0xC0 + (1)));
    clcd_putch((rx_speed % 10) + '0', (0xC0 + (2)));
}

void display_gear(uint8_t *data, uint8_t len)
{
    unsigned char rx_gear = data[0];
    clcd_print(gear_msg[rx_gear], (0xC0 + (9)));
}

void display_rpm(uint8_t *data, uint8_t len)
{
    uint16_t rx_rpm = (data[1] << 8) | data[0];

    clcd_putch((rx_rpm / 1000) + '0', (0xC0 + (4)));
    clcd_putch((rx_rpm / 100) % 10 + '0', (0xC0 + (5)));
    clcd_putch(((rx_rpm / 10) % 10) + '0', (0xC0 + (6)));
    clcd_putch((rx_rpm % 10) + '0', (0xC0 + (7)));
}

void display_indicator(uint8_t *data, uint8_t len)
{
    unsigned char rx_indicator = data[0];

    if(rx_indicator == e_ind_left)
    {
        if(led_state)
        {
            clcd_print("<-", (0xC0 + (12)));
    ...
    else if(rx_indicator == e_ind_right)
    {
        if(led_state)
        {
            clcd_print("->", (0xC0 + (14)));
    ...
    else if(rx_indicator == e_ind_hazard)
    {
        if(led_state)
        {
            clcd_print("<-", (0xC0 + (12)));
    ...
    else if(rx_indicator == e_ind_off)
    {
        clcd_print("  ", (0xC0 + (12)));
        RB0 = 0;
        clcd_print("  ", (0xC0 + (14)));
